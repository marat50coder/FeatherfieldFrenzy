...
/// Central routing brain. Given the persisted route + a fresh reachability
/// probe + attribution signals, decides which surface the user sees.
class FrenzyGateCoordinator {
  ...
  /// cleared once the pipeline finishes so a later call — e.g. Retry from
  /// the offline screen after Wi-Fi returns — runs the full pipeline again
  /// instead of replaying the cached OfflineSurface forever.
  ...
  Future<GateDestination> _decide({
    required void Function(double value) onProgress,
  }) async {
    if (!enabled) {
      ffrTrace(
        () => '[FFR.GATE] disabled '
            'runtime=$runtimeEnabled creds=${FrenzyGateConfig.grayCredentialsReady}',
      );
      onProgress(1);
    ...
    onProgress(0.12);
    final route = safe.route;
    if (route == GateRoute.unset) return _firstRun(onProgress);
    if (route == GateRoute.web) return _returningWeb(onProgress);
    return _returningGame(onProgress);
  }

  Future<GateDestination> _firstRun(void Function(double) progress) async {
    if (!await probe.hasInterface()) {
      ffrTrace(() => '[FFR.GATE] first: no interface -> offline');
    ...
    if (!await probe.canReachNetwork()) {
      ffrTrace(() => '[FFR.GATE] first: probe failed -> offline');
      return const OfflineSurface(returnToGame: false);
    }
    progress(0.48);
